...
def _generate_invalid_float(bounds: FBounds) -> float:
    ...
    if bounds.has_low and not bounds.has_high:
        return math.nextafter(bounds.low, -math.inf)

    if bounds.has_high and not bounds.has_low:
        return math.nextafter(bounds.high, math.inf)

    raise ValueError("Cannot generate invalid float: no bounds specified")
...
def _get_float_valid_borders(constraints: Sequence[Constraint]) -> FBounds:
    ...
    return FBounds(low, high, has_low, has_high)
